<template>
  <div class="login-page">
    ...
    <div class="login-card">
      ...
      <!-- Aviso personalizado para redirecionamento de agendamento -->
      <transition name="fade">
        <div v-if="avisoBloqueio" class="aviso-bloqueio">
          ...
          <div class="aviso-texto">
            <strong>Login necessário para agendar</strong>
            <p>Você precisa estar logado para agendar uma viagem. Faça login e continue de onde parou.</p>
          </div>
        </div>
      </transition>
      ...
      <div class="card-header">
        ...
        <div class="header-text">
          <h1>{{ avisoBloqueio ? 'Faça seu login' : 'Bem-vindo' }}</h1>
          <p>{{ avisoBloqueio ? 'Entre na sua conta para continuar o agendamento' : 'Acesse sua conta para continuar' }}</p>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    // Detecta se veio redirecionado da página de agendamento de viagens
    const redirect = this.$route.query.redirect || "";
    this.avisoBloqueio = redirect.includes("/viagens") || redirect.includes("/pagamento");
  },
  ...
  methods: {
    async loginUsuario() {
      ...
      try {
        ...
        sessionStorage.setItem("authToken", res.data.token || 'token-mock');
...
/* Aviso de bloqueio de agendamento */
.aviso-bloqueio {
  display: flex;
...
@keyframes pulse-border {
  ...
  50%       { border-color: rgba(99, 102, 241, 0.6); box-shadow: 0 0 0 4px rgba(99,102,241,0.06); }
}
